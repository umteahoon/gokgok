      ...
      <main className="w-full max-w-[1200px] mx-auto py-12 px-4 md:px-10 bg-white dark:bg-[#1a1a1a] rounded-t-[2.5rem] shadow-[0_-10px_40px_rgba(0,0,0,0.05)] relative z-20 -mt-24 border-t border-gray-100 dark:border-gray-800 transition-colors">
        ...
        <section className="mb-16 relative group">
          <div className="flex justify-between items-end mb-6">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                exit={{ opacity: 0, x: 10 }}
              >
                <h3 className="text-[24px] md:text-[28px] font-extrabold mb-1 tracking-tight text-gray-900 dark:text-white">
          ...
          <div className="relative w-full">
            {/* 슬라이드 화살표 (데이터가 있을 때만 노출 가능) */}
            {displayFestivals.length > 3 && (
              <>
                <button onClick={() => handleScroll("left")} className="absolute left-0 top-[35%] -translate-y-1/2 -translate-x-5 z-20 w-12 h-12 rounded-full border border-gray-200 dark:border-gray-700 flex items-center justify-center bg-white/95 dark:bg-black/95 backdrop-blur shadow-lg hover:scale-110 transition-all hidden sm:flex opacity-0 group-hover:opacity-100 text-gray-900 dark:text-white"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M15 18L9 12L15 6"/></svg></button>
                <button onClick={() => handleScroll("right")} className="absolute right-0 top-[35%] -translate-y-1/2 translate-x-5 z-20 w-12 h-12 rounded-full border border-gray-200 dark:border-gray-700 flex items-center justify-center bg-white/95 dark:bg-black/95 backdrop-blur shadow-lg hover:scale-110 transition-all hidden sm:flex opacity-0 group-hover:opacity-100 text-gray-900 dark:text-white"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M9 18l6-6-6-6"/></svg></button>
              </>
            )}

            <div ref={scrollRef} className="flex gap-4 md:gap-5 overflow-x-auto snap-x snap-mandatory pb-6 pt-2 no-scrollbar scroll-smooth">
              {displayFestivals.length > 0 ? (
                displayFestivals.map((festival) => (
                  <Link to={`/festival/${festival.id}`} key={`${activeRegion}-${festival.id}`} className="min-w-[75%] sm:min-w-[calc(33.333%-16px)] lg:min-w-[calc(25%-15px)] snap-start group/card cursor-pointer">
                ...
                <div className="w-full py-20 text-center flex flex-col items-center justify-center">
                  <p className="text-gray-400 font-bold text-lg">해당 지역의 축제 정보가 아직 없습니다 🥲</p>
                  <button onClick={() => setActiveRegion("전체")} className="mt-4 text-[#FF3478] font-bold underline underline-offset-4">전체보기로 돌아가기</button>
                </div>
              )}
            </div>
          </div>
