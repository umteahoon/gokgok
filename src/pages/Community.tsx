// 주환 - 2026.05.15: 커뮤니티 페이지 (기존 레이아웃 유지 + 사진 슬라이드 및 고화질 뷰어 추가)
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[300] bg-black/95 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <motion.img 
        ...
        className="max-w-full max-h-full object-contain pointer-events-none shadow-2xl" // object-contain으로 깨짐 방지
      />
...
const PostImageSlider = ({ images }: { images: string[] }) => {
  ...
  return (
    <div className="w-full h-full relative group/slider overflow-hidden">
      <img src={getFullImageUrl(images[idx])} className="w-full h-full object-cover transition-all" alt="Post content" />
      {images.length > 1 && (
        <>
...
const getTimeAgo = (dateString: string) => { /* 기존 로직 */ if (!dateString) return "방금 전"; const commentDate = new Date(dateString); const now = new Date(); const diffMs = now.getTime() - commentDate.getTime(); const diffMins = Math.floor(diffMs / (1000 * 60)); if (diffMins < 1) return "방금 전"; if (diffMins < 60) return `${diffMins}분 전`; if (Math.floor(diffMs / (1000 * 60 * 60)) < 24) return `${Math.floor(diffMs / (1000 * 60 * 60))}시간 전`; return `${commentDate.getFullYear()}.${String(commentDate.getMonth() + 1).padStart(2, '0')}.${String(commentDate.getDate()).padStart(2, '0')}`; };
...
export default function Community() {
  ...
  const handleCommentSubmit = async () => { /* 기존 로직 */ if (!currentUser || !commentText.trim() || !selectedPost) return; try { const res = await fetch(`https://gokgok-8ztf.onrender.com/api/community/${selectedPost.id}/comments`, { method: "POST", headers: authHeaders, body: JSON.stringify({ author: currentUser.name, author_email: currentUser.email, text: commentText }) }); const data = await res.json(); if (data.success) { const newComment = { id: data.comment.id, author: data.comment.author, author_email: data.comment.author_email, text: data.comment.text, date: data.comment.created_at, likes: 0 }; setPosts(posts.map(p => p.id === selectedPost.id ? { ...p, comments: p.comments + 1, commentsList: [...(p.commentsList || []), newComment] } : p)); setSelectedPost(prev => prev ? { ...prev, comments: prev.comments + 1, commentsList: [...(prev.commentsList || []), newComment] } : null); setCommentText(""); } } catch (e) { alert("실패"); } };
  const handleDeleteComment = async (postId: string, commentId: string) => { /* 기존 로직 */ if (!currentUser) return; if (!window.confirm("이 댓글을 삭제하시겠습니까?")) return; try { const response = await fetch(`https://gokgok-8ztf.onrender.com/api/community/${postId}/comments/${commentId}`, { method: "DELETE", headers: authHeaders, body: JSON.stringify({ author_email: currentUser.email }) }); const data = await response.json(); if (data.success) { setPosts(posts.map(p => { if (p.id === postId) { return { ...p, comments: Math.max(0, p.comments - 1), commentsList: p.commentsList?.filter(c => c.id !== commentId) }; } return p; })); if (selectedPost && selectedPost.id === postId) { setSelectedPost({ ...selectedPost, comments: Math.max(0, selectedPost.comments - 1), commentsList: selectedPost.commentsList?.filter(c => c.id !== commentId) }); } } else { alert(data.message || "삭제 권한이 없습니다."); } } catch (e) { alert("댓글 삭제 실패"); } };
  const handleDelete = async (postId: string) => { /* 기존 로직 */ if (!currentUser) return; if (!window.confirm("정말로 이 게시글을 삭제하시겠습니까?")) return; try { const response = await fetch(`https://gokgok-8ztf.onrender.com/api/community/${postId}`, { method: "DELETE", headers: authHeaders, body: JSON.stringify({ author_email: currentUser.email }) }); const data = await response.json(); if (data.success) { alert("게시글이 삭제되었습니다."); setPosts(prev => prev.filter(p => p.id !== postId)); setSelectedPost(null); } else { alert(data.message || "삭제 권한이 없습니다."); } } catch (e) { alert("삭제 처리 중 오류가 발생했습니다."); } };
  const openEditModal = (post: CommunityPost) => { setEditingPost(post); setEditTitle(post.Title); setEditContent(post.content); setEditImages(null); };
  const handleEditSubmit = async () => { /* 기존 로직 */ if (!editingPost || !currentUser) return; if (!editTitle.trim() || !editContent.trim()) return alert("제목과 내용을 모두 입력해주세요."); try { const formData = new FormData(); formData.append("author_email", currentUser.email); formData.append("title", editTitle); formData.append("content", editContent); if (editImages && editImages.length > 0) { Array.from(editImages).forEach(file => formData.append("images", file)); } else { editingPost.images.forEach(img => formData.append("images", img)); } const response = await fetch(`https://gokgok-8ztf.onrender.com/api/community/${editingPost.id}`, { method: "PUT", headers: formDataHeaders, body: formData }); const data = await response.json(); if (data.success) { alert("게시글이 성공적으로 수정되었습니다."); const updatedImages = data.post.images || editingPost.images; setPosts(prev => prev.map(p => p.id === editingPost.id ? { ...p, Title: editTitle, content: editContent, images: updatedImages } : p)); if (selectedPost && selectedPost.id === editingPost.id) { setSelectedPost(prev => prev ? { ...prev, Title: editTitle, content: editContent, images: updatedImages } : null); } setEditingPost(null); } else { alert(data.message || "수정 권한이 없습니다."); } } catch (e) { alert("수정 처리 중 오류가 발생했습니다."); } };
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#111111] transition-colors relative font-sans text-[#111111] dark:text-white pb-20">
                ...
                >
                  <div className="flex flex-col flex-1">
                      ...
                      {post.images.length > 0 ? (
                        <>
                          ...
                          {post.images.length > 1 && (
                            <div className="absolute bottom-2 right-2 bg-black/70 text-white text-[10px] px-2 py-1 rounded-md font-bold backdrop-blur-sm">+{post.images.length - 1}</div>
                          )}
                        </>
                      ) : (
                    ...
                    <div className="flex items-center justify-between mb-3 text-left">
                      ...
                      <Badge variant="outline" className="border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-600 dark:text-gray-300 font-bold text-[10px] px-2 py-0.5 rounded-full">{post.category}</Badge>
                    </div>
      ...
      {/* 💡 [추가] 고화질 원본 이미지 뷰어 (AnimatePresence 외부) */}
      <AnimatePresence>
        ...
        {selectedPost && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-0 md:p-10" onClick={() => setSelectedPost(null)}>
            <motion.div initial={{ scale: 0.95, y: 30 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 30 }} onClick={(e) => e.stopPropagation()} className="bg-white dark:bg-[#1a1a1a] w-full h-full md:h-[85vh] max-w-5xl md:rounded-[2.5rem] shadow-2xl flex flex-col md:flex-row overflow-hidden relative">
              ...
              {/* 💡 [수정] 모달 이미지 영역: 슬라이더 적용 (해상도 깨짐 방지) */}
      ...
      <AnimatePresence>{editingPost && ( <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] flex items-center justify-center bg-black/60 backdrop-blur-md p-4"><motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white dark:bg-[#1a1a1a] w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col border border-gray-100 dark:border-gray-800"><div className="p-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-white dark:bg-[#1a1a1a]"><h3 className="font-extrabold text-xl text-gray-900 dark:text-white tracking-tight">게시글 수정</h3><button onClick={() => setEditingPost(null)} className="text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors"><X className="w-6 h-6" /></button></div><div className="p-8 flex flex-col gap-6 text-left"><div><label className="text-[13px] font-extrabold text-gray-900 dark:text-gray-200 mb-2 block">제목</label><input value={editTitle} onChange={e => setEditTitle(e.target.value)} className="w-full bg-gray-50 dark:bg-[#222] border border-gray-200 dark:border-gray-700 rounded-2xl p-4 font-bold text-gray-900 dark:text-white outline-none focus:bg-white dark:focus:bg-[#1a1a1a] transition-all shadow-sm" placeholder="제목을 입력하세요" /></div><div><label className="text-[13px] font-extrabold text-gray-900 dark:text-gray-200 mb-2 block">내용</label><textarea value={editContent} onChange={e => setEditContent(e.target.value)} className="w-full h-48 bg-gray-50 dark:bg-[#222] border border-gray-200 dark:border-gray-700 rounded-2xl p-4 font-medium text-gray-700 dark:text-gray-300 outline-none focus:bg-white dark:focus:bg-[#1a1a1a] resize-none transition-all shadow-sm" placeholder="내용을 입력하세요" /></div><div><label className="text-[13px] font-extrabold text-gray-900 dark:text-gray-200 mb-2 block">사진 변경</label><input type="file" multiple accept="image/*" onChange={(e) => setEditImages(e.target.files)} className="text-sm file:mr-4 file:py-3 file:px-6 file:rounded-full file:border-0 file:bg-gray-900 file:text-white" /></div></div><div className="p-6 border-t border-gray-100 dark:border-gray-800 flex justify-end gap-3 bg-white dark:bg-[#1a1a1a]"><Button variant="outline" className="font-bold rounded-full px-6" onClick={() => setEditingPost(null)}>취소</Button><Button className="bg-gray-900 dark:bg-white hover:bg-black dark:hover:bg-gray-200 text-white dark:text-black font-bold px-8 rounded-full" onClick={handleEditSubmit}>수정 완료</Button></div></motion.div></motion.div> )}</AnimatePresence>
    </div>
  );
}