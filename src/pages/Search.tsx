...
export default function Search() {  
  ...
  const bestScrollRef = useRef<HTMLDivElement>(null); const risingScrollRef = useRef<HTMLDivElement>(null); const tasteScrollRef = useRef<HTMLDivElement>(null);
  ...
  return (
    ...
    <div className="w-full bg-white dark:bg-[#111111] text-[#111111] dark:text-white pb-20 font-sans overflow-x-hidden pt-10 rounded-t-3xl transition-colors">
      <main className="container mx-auto">
        ...
        <div className="px-4">
          <div className="flex gap-6 mb-10 border-b border-gray-100 dark:border-gray-800">
            <button onClick={() => setActiveTab("list")} className={`pb-3 text-lg font-bold transition-colors relative ${activeTab === "list" ? "text-[#FF3478]" : "text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300"}`}>목록보기 {activeTab === "list" && <motion.div layoutId="t-line" className="absolute bottom-0 left-0 right-0 h-[3px] bg-[#FF3478]" />}</button>
            <button onClick={() => setActiveTab("map")} className={`pb-3 text-lg font-bold transition-colors relative ${activeTab === "map" ? "text-[#FF3478]" : "text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300"}`}>지도보기 {activeTab === "map" && <motion.div layoutId="t-line" className="absolute bottom-0 left-0 right-0 h-[3px] bg-[#FF3478]" />}</button>
          </div>
            ...
            {activeTab === "list" ? (
              <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  <div ref={risingScrollRef} className="flex gap-4 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] snap-x snap-mandatory pb-4">
                    {mockFestivals.map((festival) => (
                      <Link to={`/festival/${festival.id}`} key={`rising-${festival.id}`} className="min-w-[calc(50%-10px)] md:min-w-[calc(20%-12.8px)] snap-start group/card cursor-pointer">
                        ...
                        <div className="px-1"><h3 className="font-bold text-[14.5px] line-clamp-2 h-[40px] text-gray-900 dark:text-white group-hover/card:text-[#FF3478] transition-colors">{festival.title}</h3><p className="text-[12px] text-gray-400 dark:text-gray-500">{festival.date}</p></div>
                      </Link>
                    ))}
                  </div>
                </section>
                ...
                <section className="mb-20 relative group">
                  ...
                  <div className="flex gap-2 mb-8 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">{tastes.map((taste) => (<button key={taste} onClick={() => setSelectedTaste(taste)} className={`px-4 py-2 rounded-full text-sm font-bold border transition-all shrink-0 ${selectedTaste === taste ? "bg-[#111111] dark:bg-white text-white dark:text-[#111111] border-[#111111] dark:border-white" : "bg-[#F5F5F5] dark:bg-[#222222] text-[#666666] dark:text-gray-400 border-transparent"}`}>{taste}</button>))}</div>
                  <div ref={tasteScrollRef} className="flex gap-5 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] snap-x snap-mandatory pb-4">
                    {filteredList.map((festival) => (
                      <Link to={`/festival/${festival.id}`} key={`holic-${festival.id}`} className="w-[280px] md:w-[320px] flex-shrink-0 snap-start group/card cursor-pointer">
                        <div className="relative mb-4 aspect-[1.4/1] rounded-2xl overflow-hidden shadow-md bg-gray-100 dark:bg-[#222222]"><img src={festival.image} alt={festival.title} className="w-full h-full object-cover transition-transform duration-500 group-hover/card:scale-105" /><div className="absolute top-3 left-3 flex flex-col gap-1"><span className={`px-2 py-0.5 rounded text-[10px] font-bold text-white w-fit ${festival.status === 'ended' ? 'bg-gray-500/80' : 'bg-[#FF3478]/90'} backdrop-blur-sm shadow-md`}>{getStatusLabel(festival.status || "")}</span></div></div>
                  ...
                  {themes.map((theme, idx) => {
                    ...
                    return (
                      <div key={idx} className={`border rounded-2xl overflow-hidden transition-all duration-300 ${isExpanded ? "border-blue-500 ring-1 ring-blue-500 shadow-md" : "border-gray-200 dark:border-gray-800 shadow-sm"}`}>
                        <button onClick={() => setExpandedIndex(isExpanded ? null : idx)} className="w-full px-6 py-5 flex justify-between items-center bg-white dark:bg-[#1a1a1a] transition-colors"><span className={`text-lg font-bold ${isExpanded ? "text-gray-900 dark:text-white" : "text-gray-500 dark:text-gray-400"}`}>{theme.title}</span><ChevronLeft className={`w-6 h-6 transition-transform duration-300 ${isExpanded ? "-rotate-90 text-blue-500" : "rotate-0 text-gray-400 dark:text-gray-500"}`} /></button>
                        <AnimatePresence>
                          {isExpanded && (
                                ...
                                <div className="flex gap-4 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] pb-2 snap-x">
                                  {theme.items.map((item) => (
                                    <Link to={`/festival/${item.id}`} key={item.id} className="min-w-[160px] md:min-w-[200px] snap-start group/item relative">
                                      <div className="relative aspect-[3/4] rounded-2xl overflow-hidden mb-2"><img src={item.image} alt={item.title} className="w-full h-full object-cover transition-transform duration-500 group-hover/item:scale-110" /><div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex items-end p-4"><p className="text-white font-bold text-sm leading-tight line-clamp-2">{item.title}</p></div></div>
                                    </Link>
                                  ))}
                                </div>
                              </div>
              ...
              <motion.div key="map-view" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 lg:grid-cols-12 gap-8 min-h-[650px]">
                ...
                <div className="lg:col-span-7 bg-[#F8F9FA] dark:bg-[#1a1a1a] rounded-[3rem] flex items-center justify-center border border-gray-100 dark:border-[#333] overflow-hidden relative shadow-inner p-4">
                   ...
                   <div className="absolute bottom-6 left-8 bg-white/80 dark:bg-black/80 backdrop-blur-md px-5 py-2.5 rounded-full border border-gray-100 dark:border-gray-800 shadow-sm flex items-center gap-2">
                     ...
                     {selectedRegion ? (
                       <motion.div key="region-results" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="flex flex-col h-full">
                         <div className="flex justify-between items-end mb-6 px-2">
                           <h3 className="text-2xl font-black text-gray-900 dark:text-white">{REGION_NAME_MAP[selectedRegion]} 축제</h3>
                           <span className="text-sm font-bold text-[#FF3478] bg-[#FF3478]/5 dark:bg-[#FF3478]/20 px-3 py-1 rounded-full">총 {regionFestivals.length}건</span>
                         </div>
                             ...
                             regionFestivals.map((festival) => (
                               <Link to={`/festival/${festival.id}`} key={`map-item-${festival.id}`} className="group flex gap-4 bg-white dark:bg-[#222] p-3 rounded-[2rem] border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-md transition-all active:scale-[0.98]">
                                 ...
                                 <div className="flex flex-col justify-center overflow-hidden">
                                   ...
                                   <p className="text-[11px] text-gray-400 dark:text-gray-500 mt-1 font-medium">{festival.date}</p>
                                 </div>
                               </Link>
                             ))
                           ) : (
                             <div className="h-full flex flex-col items-center justify-center text-center bg-gray-50 dark:bg-[#1a1a1a] rounded-[2.5rem] border border-dashed border-gray-200 dark:border-gray-800 p-8">
                               <SearchIcon className="w-10 h-10 text-gray-300 dark:text-gray-600 mb-3" />
                               <p className="text-gray-400 dark:text-gray-500 font-medium text-sm">해당 지역에 등록된 축제가 없습니다.</p>
                             </div>
                           )}
                         </div>
                       </motion.div>
                       ...
                       <div className="h-full flex flex-col items-center justify-center text-center bg-gray-50 dark:bg-[#1a1a1a] rounded-[2.5rem] border border-dashed border-gray-200 dark:border-gray-800 p-10">
                         ...
                         <p className="text-gray-400 dark:text-gray-500 text-sm">왼쪽 지도에서 궁금한 지역을 클릭하면<br/>해당 지역의 축제 정보를 한눈에 볼 수 있습니다!</p>
                       </div>
                     )}
                   </AnimatePresence>
                </div>
