      ...
      <main className="container mx-auto">
        ...
        <section className="relative w-[calc(100%-2rem)] mx-auto h-[250px] md:h-[380px] mb-16 bg-black overflow-hidden rounded-[2.5rem] md:rounded-[3.5rem] shadow-xl">
          ...
          <div className="absolute bottom-8 right-10 z-20 flex items-center bg-black/40 backdrop-blur-lg px-5 py-2 rounded-full border border-white/10 text-white shadow-2xl">
            ...
            <span className="text-xs font-bold tracking-[0.2em] font-mono">
        ...
        <div className="px-4">
          <div className="flex items-center justify-between mb-10 border-b border-gray-100 dark:border-gray-800 relative">
            ...
            <button 
              ...
              className={`pb-3 px-2 transition-colors ${isSearchOpen ? "text-[#FF3478]" : "text-gray-400 hover:text-gray-600 dark:hover:text-gray-200"}`}
            >
            ...
            {isSearchOpen && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-12">
                <div className="relative max-w-3xl mx-auto">
                  ...
                  <SearchIcon className="absolute right-8 top-1/2 -translate-y-1/2 text-gray-400" size={24} />
                </div>
              </motion.div>
            )}
          </AnimatePresence>
            ...
            {activeTab === "list" ? (
              <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                  ...
                  <section className="mb-16">
                    ...
                    {searchResults.length > 0 ? (
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {searchResults.map((f) => (
                          <Link key={f.id} to={`/festival/${f.id}`} className="group flex gap-4 bg-white dark:bg-[#222] p-3 rounded-[1.5rem] border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-md transition-all active:scale-[0.98]">
                      ...
                      <div className="py-20 text-center bg-gray-50 dark:bg-[#1a1a1a] rounded-[2.5rem] border border-dashed border-gray-200 dark:border-gray-800 text-gray-400 font-bold">일치하는 축제를 찾지 못했습니다 🥲</div>
                    )}
                  </section>
                )}
                ...
                <section className="mb-20">
                  <h2 className="text-[22px] font-extrabold italic underline decoration-[#FF3478]/20 underline-offset-8 text-gray-900 dark:text-white mb-8">TOP! 베스트 축제</h2>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-4 gap-y-10">
                    {topFestivals.map((festival, idx) => (
                      <Link to={`/festival/${festival.id}`} key={`best-${festival.id}`} className="group/card cursor-pointer">
                        ...
                        <div className="px-1"><h3 className="font-bold text-[15px] line-clamp-1 text-gray-900 dark:text-white group-hover/card:text-[#FF3478]">{festival.title}</h3><p className="text-[12px] text-gray-400 mt-1">{festival.location}</p></div>
                      </Link>
                    ))}
                  </div>
                </section>
                ...
                {/* 섹션 2: 인기 급상승 🔥 */}
                <section className="mb-20">
                  <h2 className="text-[22px] font-extrabold text-gray-900 dark:text-white mb-8">지금 인기 급상승 🔥</h2>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-4 gap-y-10">
                    {mockFestivals.map((festival) => (
                      <Link to={`/festival/${festival.id}`} key={`rising-${festival.id}`} className="group/card cursor-pointer">
                        <div className="relative mb-3 aspect-[4/5] rounded-xl overflow-hidden shadow-sm bg-gray-50 dark:bg-[#222]">
                          <img src={festival.image} alt={festival.title} className="w-full h-full object-cover transition-transform duration-500 group-hover/card:scale-110" />
                ...
                {/* 섹션 3: 취향에 따라 고르는 여행 */}
                <section className="mb-20">
                  <h2 className="text-[22px] font-bold text-gray-900 dark:text-white mb-8">취향에 따라 고르는 여행</h2>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                    {filteredList.map((festival) => (
                      <Link to={`/festival/${festival.id}`} key={`holic-${festival.id}`} className="group/card cursor-pointer">
                        ...
                        <h3 className="font-bold text-[18px] text-gray-900 dark:text-white group-hover/card:text-[#FF3478]">{festival.title}</h3>
                            ...
                            <div className="px-8 pb-10 pt-4">
                              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
                                {theme.items.map((item) => (
                                  <Link to={`/festival/${item.id}`} key={item.id} className="group/item relative aspect-[3/4] rounded-2xl overflow-hidden shadow-sm"><img src={item.image} alt={item.title} className="w-full h-full object-cover transition-transform duration-500 group-hover/item:scale-110" /><div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex items-end p-4"><p className="text-white font-bold text-xs leading-tight line-clamp-2">{item.title}</p></div></Link>
                                ))}
                              </div>
                            </div>
                          </motion.div>
