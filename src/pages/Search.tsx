...
import axios from "axios"; // 실서버 통신을 위해 axios 주입
...
export default function Search() {
  ...
  // 🚀 [패치] 백엔드 실서버 데이터 통합 로드 파이프라인
  useEffect(() => {
    const loadServerData = async () => {
      try {
        setLoading(true);
        // 우리가 업데이트한 백엔드의 축제 마스터 API 호출
        const response = await axios.get("https://gokgok-8ztf.onrender.com/api/festivals");
        
        if (response.data && response.data.success) {
          // 데이터가 잘 왔으면 Supabase 실서버 축제 데이터셋 바인딩
          setDbFestivals(response.data.data);
        }
      } catch (err) {
        console.error("❌ 실서버 축제 데이터 연동 실패 (기본 로컬 모드로 우회):", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  // 🎯 [패치] 테마 아코디언용 데이터를 실서버 실시간 데이터 카테고리로 재매핑
      ...
      <main className="container mx-auto">
        ...
        {loading ? (
          <div className="w-full h-[300px] flex flex-col items-center justify-center">
            ...
            <p className="text-gray-400 text-sm mt-4 font-medium">곡곡 엔진 실시간 축제 데이터 로드 중...</p>
          </div>
        ) : (
          <>
            {/* 히어로 슬라이더 (실서버 가동) */}
              ...
              <section className="relative w-[calc(100%-2rem)] mx-auto h-[220px] md:h-[320px] mb-16 bg-black overflow-hidden rounded-[2.5rem] md:rounded-[3.5rem] shadow-xl">
                ...
                <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 flex items-center bg-black/40 backdrop-blur-lg px-4 py-1.5 rounded-full border border-white/5">
                  <div className="text-white text-[10px] font-bold tracking-widest">
                    {String(currentSlide + 1).padStart(2, "0")} / {String(heroItems.length).padStart(2, "0")}
                  </div>
                </div>
              </section>
            )}
            ...
            <div className="w-[calc(100%-2rem)] mx-auto">
              ...
              <div className="flex items-center justify-between mb-10 border-b border-gray-100 dark:border-gray-800 relative">
                <div className="flex gap-6">
                  ...
                  >
                    목록보기 {activeTab === "list" && <div className="absolute bottom-0 left-0 right-0 h-[3px] bg-[#FF3478]" />}
                  </button>
                  <button
                    onClick={() => setActiveTab("map")}
                    className={`pb-3 text-lg font-bold transition-colors relative ${activeTab === "map" ? "text-[#FF3478]" : "text-gray-400"}`}
                  >
                    지도보기 {activeTab === "map" && <div className="absolute bottom-0 left-0 right-0 h-[3px] bg-[#FF3478]" />}
                  </button>
                </div>
                <button
                  onClick={() => {
                  ...
                  >
                    ...
                    {searchQuery && (
                      <section className="mb-16">
                        <h2 className="text-xl font-black mb-8 px-2">
                          검색 결과 <span className="text-[#FF3478]">{searchResults.length}</span>
                        </h2>
                            ...
                            <div className="relative mb-3 aspect-[4/5] rounded-xl overflow-hidden shadow-sm bg-gray-50 dark:bg-[#222222]">
                              ...
                              <div onClick={(e) => toggleWishlist(e, festival.id)} className="absolute top-2 right-2 z-10 p-1 cursor-pointer" style={heartBtnStyle}>
                                <motion.div whileTap={{ scale: 0.7 }}>
                                  <Heart className={`w-6 h-6 drop-shadow-md transition-colors ${wishlistedIds.includes(String(festival.id)) ? "fill-[#FF3478] text-[#FF3478]" : "text-white/70 hover:text-white"}`} />
                                </motion.div>
                              </div>
                              <div className="absolute top-2 left-2">
                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold text-white ${festival.status === "ended" ? "bg-gray-500/80" : "bg-[#FF3478]/90"} backdrop-blur-sm shadow-md`}>{getStatusLabel(festival.status || "")}</span>
                              </div>
                      ...
                      <div ref={risingScrollRef} className="flex gap-4 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] snap-x snap-mandatory pb-4">
                        {[...dbFestivals].reverse().slice(0, 10).map((festival) => (
                          <Link to={`/festival/${festival.id}`} key={`rising-${festival.id}`} className="min-w-[calc(50%-10px)] md:min-w-[calc(25%-12px)] lg:min-w-[calc(20%-12px)] snap-start group/card cursor-pointer">
                            ...
                            <div className="px-1">
                              <h3 className="font-bold text-[14.5px] line-clamp-2 h-[40px] text-gray-900 dark:text-white group-hover/card:text-[#FF3478] transition-colors">{festival.title}</h3>
                            </div>
                          </Link>
                        ))}
                      </div>
                    ...
                    <section className="mb-20 relative group">
                      ...
                      <div className="flex gap-2 mb-8 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                        {tastes.map((taste) => (
                          <button key={taste} onClick={() => setSelectedTaste(taste)} className={`px-4 py-2 rounded-full text-sm font-bold border transition-all shrink-0 ${selectedTaste === taste ? "bg-[#111111] dark:bg-white text-white dark:text-[#111111] border-[#111111] dark:border-white" : "bg-[#F5F5F5] dark:bg-[#222222] text-[#666666] border-transparent"}`}>{taste}</button>
                        ))}
                      </div>
                        ...
                        <div className="flex flex-col h-full px-2">
                          ...
                          <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] space-y-4 pr-2">
                            {regionFestivals.length > 0 ? (
                              regionFestivals.map((festival) => (
                                <Link to={`/festival/${festival.id}`} key={`map-item-${festival.id}`} className="group flex gap-4 bg-white dark:bg-[#222] p-3 rounded-[2rem] border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-md transition-all active:scale-[0.98]">
                                  <div className="w-24 h-24 shrink-0 rounded-2xl overflow-hidden shadow-sm">
                                    <img src={festival.image} className="w-full h-full object-cover group-hover:scale-110 transition-transform" />
                                  </div>
                                  <div className="flex flex-col justify-center overflow-hidden">
                              ...
                              <div className="text-center py-12 text-gray-400 font-medium">해당 지역에 등록된 실실시간 축제가 없습니다.</div>
                            )}
                          </div>
                        </div>
                      ) : (
