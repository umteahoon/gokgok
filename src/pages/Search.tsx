  ...
  return (
    <div className="w-full bg-white dark:bg-[#111111] text-[#111111] dark:text-white pb-20 font-sans overflow-x-hidden pt-10 rounded-t-3xl transition-colors">
      <main className="container mx-auto">
        ...
        {/* --- [수정] 상단 히어로 배너: 야놀자 스타일 정렬 --- */}
        <section className="relative w-full mb-16 overflow-hidden">
          <motion.div 
            className="flex items-center"
            style={{ paddingLeft: "7.5%", gap: "12px" }}
            animate={{ x: `calc(-${currentSlide * 85}% - ${currentSlide * 12}px)` }}
            ...
            {heroItems.map((item, idx) => (
              <div 
                ...
                className={`relative shrink-0 w-[85%] h-[280px] md:h-[420px] rounded-[2.5rem] md:rounded-[3.5rem] overflow-hidden shadow-xl transition-all duration-500 ${
                  currentSlide === idx ? 'scale-100 opacity-100' : 'scale-[0.95] opacity-40 blur-[1px]'
                }`}
              >
          ...
          <button onClick={() => setCurrentSlide((p) => (p - 1 + heroItems.length) % heroItems.length)} className="absolute left-[10%] top-1/2 -translate-y-1/2 z-30 w-11 h-11 bg-white/10 hover:bg-white/30 backdrop-blur-md rounded-full flex items-center justify-center text-white transition-all"><ChevronLeft size={28} /></button>
          <button onClick={() => setCurrentSlide((p) => (p + 1) % heroItems.length)} className="absolute right-[10%] top-1/2 -translate-y-1/2 z-30 w-11 h-11 bg-white/10 hover:bg-white/30 backdrop-blur-md rounded-full flex items-center justify-center text-white transition-all"><ChevronRight size={28} /></button>
        </section>
              ...
              <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                ...
                {searchQuery && (
                  <section className="mb-16">
                    <h2 className="text-xl font-black mb-8">검색 결과 <span className="text-[#FF3478]">{searchResults.length}</span></h2>
                ...
                <section className="mb-20">
                  ...
                  <div ref={tasteScrollRef} className="flex gap-6 overflow-x-auto no-scrollbar snap-x snap-mandatory pb-4">
                      ...
                      <AnimatePresence>
                        {expandedIndex === idx && (
                          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="bg-gray-50/50 dark:bg-black/20">
                            <div className="px-8 pb-10 pt-4 overflow-x-auto no-scrollbar"><div className="flex gap-4">{theme.items.map((item) => (<Link to={`/festival/${item.id}`} key={item.id} className="min-w-[150px] md:min-w-[200px] group/item relative aspect-[3/4] rounded-2xl overflow-hidden shadow-sm"><img src={item.image} alt={item.title} className="w-full h-full object-cover transition-transform duration-500 group-hover/item:scale-110" /><div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex items-end p-4"><p className="text-white font-bold text-xs leading-tight line-clamp-2">{item.title}</p></div></Link>))}</div></div>
                          </motion.div>
                        )}
                      </AnimatePresence>
                    </div>
