...
import { 
  getCurrentUser, 
  logout, 
  resetPassword, // 아까 만든 비밀번호 재설정 함수
  deleteAccount, 
  updateProfilePhoto, 
...
export default function MyPage() {
  ...
  useEffect(() => {
    const loadUserData = async () => {
      ...
      try {
        // 1. 찜 목록 불러오기 (user.id 또는 user.email 기반)
        ...
        <motion.div className="container mx-auto px-4" initial="hidden" animate="visible" variants={fadeInUp}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl">
            <Card className="border-none shadow-sm">
              ...
              <CardContent><p className="text-4xl font-bold text-primary">{isLoading ? '...' : savedFestivals.length}</p></CardContent>
            </Card>
            <Card className="border-none shadow-sm">
              <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">작성한 리뷰</CardTitle></CardHeader>
              <CardContent><p className="text-4xl font-bold text-primary">{isLoading ? '...' : reviewCount}</p></CardContent>
            </Card>
          </div>
        </motion.div>
      </div>
      ...
      <div className="container mx-auto px-4 py-12">
        <Tabs defaultValue="saved" className="w-full">
          <TabsList className="flex w-full max-w-md border-b bg-transparent h-auto p-0 mb-10 gap-8">
            <TabsTrigger value="saved" className="px-2 py-3 border-b-2 border-transparent data-[state=active]:border-primary rounded-none shadow-none bg-transparent">찜한 축제</TabsTrigger>
            <TabsTrigger value="settings" className="px-2 py-3 border-b-2 border-transparent data-[state=active]:border-primary rounded-none shadow-none bg-transparent">설정</TabsTrigger>
          </TabsList>

          <TabsContent value="saved">
            {isLoading ? (
              <div className="text-center py-20">정보를 불러오는 중입니다...</div>
              ...
              <div className="text-center py-20 bg-gray-50 rounded-2xl border-2 border-dashed">
                <p className="text-gray-500">아직 찜한 축제가 없습니다.</p>
              </div>
            )}
          </TabsContent>
            ...
            <div className="max-w-2xl space-y-6">
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-1"><Label>이름</Label><Input value={currentUser.name} disabled /></div>
                  <div className="space-y-1"><Label>이메일</Label><Input value={currentUser.email} disabled /></div>
                </CardContent>
              </Card>
              <Card className="border-red-100">
                <CardHeader><CardTitle className="text-red-600">위험 구역</CardTitle></CardHeader>
